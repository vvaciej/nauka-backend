  ...
  return (
		<div className='w-full h-full flex justify-center'>
			<header className='h-20 w-5/6 flex items-center justify-between'>
				<div>
					<Link className='text-2xl text-blue-200' href={'/'}>
						Nauka backend
					</Link>
				</div>
				<div className='flex gap-6 items-center'>
					{user ? (
						<>
							<Link href={'/password'} className='font-bold'>
								{user.email}
							</Link>
							<Link href={'/your-informations'} className='px-4 py-2 rounded-full text-white font-normal hover:bg-white hover:text-black transition-colors duration-100 cursor-pointer'>
								Moje dane
							</Link>
							<button
								onClick={logoutClick}
